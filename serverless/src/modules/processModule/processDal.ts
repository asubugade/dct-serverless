...
import GenProcess from "../../models/GenProcess"
...
export const getProcessCount = async (cSearchFilterQuery, cStatusFilter) => {
    try {
        ...
        const countResult = await GenProcess.aggregate(countAggregatePipeline);
...
export const getProcess = async (aRequestDetails, oSort, cSearchFilterQuery, iCount, cStatusFilter) => {
    ...
    const oDocuments = await GenProcess.aggregate([
        cSearchFilterQuery,
        {
            $lookup: {
                from: "gen_statuses",
