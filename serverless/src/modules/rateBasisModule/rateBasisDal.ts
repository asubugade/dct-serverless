...
export const getRateBasisCount = async(cSearchFilterQuery) =>{    
    try {
      const countAggregatePipeline = [
        cSearchFilterQuery,
        {
          $lookup: {
            ...
            foreignField: "_id",
            as: "oRatebasisListing"
          }
        },
        { $unwind: "$oRatebasisListing" },
        {
          $group: {
            _id: null,
            count: { $sum: 1 }
      ...
      const countResult = await GenRatebasis.aggregate(countAggregatePipeline);
  ...
  export const getRateBasis = async(aRequestDetails, oSort, cSearchFilterQuery, iCount)=> {
    ...
    const oDocuments = await GenRatebasis.aggregate([
      cSearchFilterQuery,
      {
        $lookup: {
          from: "gen_statuses",
