...
export const getLocationCount = async (cSearchFilterQuery) => {
    try {
        ...
        const countResult = await Location.aggregate(countAggregatePipeline);
...
export const getLocation = async (aRequestDetails, oSort, cSearchFilterQuery, iCount) => {
    ...
    const oDocuments = await Location.aggregate([
        cSearchFilterQuery,
        {
            $lookup: {
                from: "gen_statuses",
